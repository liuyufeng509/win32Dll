...
COtlDbTool * COtlDbTool::m_pInstance = NULL;
COtlDbTool::COtlDbTool(void):isInited(false)
{
...
int COtlDbTool::query(int tid)
{
	...
	try
	{
		otl_stream i ( 1 , // buffer size
                  "select * from model_view1 where rownum<1000", // SELECT statement
	  ...
	  {//循环取出每一行
		  ...
		  k++;
		  //cout<<"read the "<<k<<" recorde, tid="<<tid<<endl;
	  }
	}catch(otl_exception& p)
	{
		cerr<<p.msg<<endl; // print out error message
